...
| `architecture/intent-detection.md` | Three routing layers: command routing, intent detection (heuristic), content classification (LLM) | intent, routing, commands, classification, gateway |
| `architecture/sessions.md` | Session management: topic-based, STM lifecycle, CRUD operations | session, stm, topic, load, clear |
| `architecture/output-routing.md` | Output routing rules: which channel for which output type | routing, discord, web, artifact, notification |
| `architecture/vault-seed.md` | Vault seed workflow: markdown → embed → Postgres, file format, when to run | vault, seed, radars, signals, embed |
